...
public class Modelo {
	...
	public String req3String(String pais1, String pais2) {
		...
		while (!pila.isEmpty()) {
			...
			distancia = dist.distancia(longdestino, latdestino, longorigen, latorigen);
	...
	public String req4String() {
		...
		try {
			...
			Comparator<Vertex<String, Landing>> comparadorVertice = new Vertex.ComparadorXKey();
			ILista unificado = merge.unificarListas(candidatos, comparadorVertice);
	...
	public ILista req5(String punto) {
			...
			try {
				...
				for (int j = 1; j <= arcos.size(); j++) {
					...
					if (vertice2.getInfo().getClass().getName().equals("model.data_structures.Landing")) {
						...
						float distancia = dist.distancia(pais.getLongitude(), pais.getLatitude(),
		...
		Comparator<Country> comparadorCountry = new Country.ComparadorXNombre();
		ILista unificado = merge.unificarListas(countries, comparadorCountry);
	...
	public void cargar() throws IOException {
		...
		for (CSVRecord record3 : records3) {
			...
			if (pais1 != null) {
				float weight = dist.distancia(pais1.getLongitude(), pais1.getLatitude(), landing1.getLongitude(),
			...
			if (pais2 != null) {
				float weight2 = dist.distancia(pais2.getLongitude(), pais2.getLatitude(), landing1.getLongitude(),
						landing1.getLatitude());
			...
			if (landing1 != null) {
				if (landing2 != null) {
					...
					if (existe1 == null) {
						float weight3 = dist.distancia(landing1.getLongitude(), landing1.getLatitude(),
